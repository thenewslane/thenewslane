...
def run_pipeline_webhook():
    """Accept POST to trigger one pipeline run. Returns 202 immediately; run is in background."""
    global _pipeline_running
    ...
    thread.start()
    log.info("Webhook: pipeline run started in background")
    return jsonify({"ok": True, "triggered": True}), 202
